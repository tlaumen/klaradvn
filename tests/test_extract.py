...
def test_extract_function_code():
    f, path = extract_function_code(str(Path(__file__).parent / 'test-package'), 'lorem_ipsum')
    val = """def lorem_ipsum():
    a = 2
    if a > 3:
...
def test_extract_another_function():
    f, _ = extract_function_code(str(Path(__file__).parent / 'test-package'), 'another_function')
